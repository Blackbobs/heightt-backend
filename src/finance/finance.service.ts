...
export class FinanceService {
  ...
  // ============================================
  // PAYMENT PROCESSING (WITH ROW LOCKING)
  // ============================================
  ...
  async processPayment(userId: string, dto: CreatePaymentDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      this.logger.log(
        `Payment processed: ${payment.id} - ${dto.amount} Kobo (₦${(dto.amount / this.KOBO_PER_NAIRA).toFixed(2)})`,
      );
      return {
        payment,
        transaction,
  ...
  async rejectOrganizationWithdrawal(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (withdrawal.walletId) {
        ...
        if (wallet) {
          const refundBalanceBefore = wallet.balance;
          const refundBalanceAfter = refundBalanceBefore + withdrawal.amount;

          await tx.wallet.update({
            where: { id: withdrawal.walletId },
            data: { balance: refundBalanceAfter },
          });
          ...
          const refundTransaction = await tx.transaction.create({
            data: {
              ...
              status: 'COMPLETED',
              reference: `REF_${randomBytes(16).toString('hex').toUpperCase()}`,
...
/**
 * Get financial overview for a specific organization
 */
async getOrganizationFinancialOverview(organizationId: string, userId: string) {
  this.logger.log(`Getting financial overview for organization: ${organizationId}`);
  ...
  if (!membership) {
    throw new ForbiddenException('You do not have access to this organization\'s financial data');
  }
  ...
  return {
    ...
    dueStats: {
      ...
      collectionRate: totalDueAmountValue > 0 
...
/**
 * Get monthly revenue breakdown for an organization
 */
...
/**
 * Get comprehensive finance dashboard for an organization
 */
async getOrganizationFinanceDashboard(organizationId: string, userId: string) {
  this.logger.log(`Getting finance dashboard for organization: ${organizationId}`);
  ...
  return {
    ...
    overdueDues: overdueDues.map(da => ({
      ...
      dueName: da.due?.name,
      dueDate: da.due?.dueDate,
      daysOverdue: Math.floor((new Date().getTime() - new Date(da.due?.dueDate || new Date()).getTime()) / (1000 * 60 * 60 * 24)),
    })),
    duesDueSoon: duesDueSoon.map(da => ({
      ...
      dueName: da.due?.name,
      dueDate: da.due?.dueDate,
      daysUntilDue: Math.floor((new Date(da.due?.dueDate || new Date()).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)),
    })),
  };
}
}
