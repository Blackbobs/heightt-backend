...
const PERMISSION_CATALOG = [
  ...
  {
    key: 'academic_level:create',
    name: 'Create academic levels',
    category: 'ACADEMIC_LEVEL',
  },
  {
    key: 'academic_level:read',
    name: 'View academic levels',
    category: 'ACADEMIC_LEVEL',
  },
  {
    key: 'academic_level:update',
    name: 'Update academic levels',
    category: 'ACADEMIC_LEVEL',
  },
  {
    key: 'academic_level:delete',
    name: 'Delete academic levels',
    category: 'ACADEMIC_LEVEL',
  },
  {
    key: 'academic_session:create',
    name: 'Create academic sessions',
    category: 'ACADEMIC_SESSION',
  },
  {
    key: 'academic_session:read',
    name: 'View academic sessions',
    category: 'ACADEMIC_SESSION',
  },
  {
    key: 'academic_session:update',
    name: 'Update academic sessions',
    category: 'ACADEMIC_SESSION',
  },
  {
    key: 'academic_session:delete',
    name: 'Delete academic sessions',
    category: 'ACADEMIC_SESSION',
  },
...
const FINANCE_PERMISSIONS = [
  ...
  {
    key: 'finance:manual',
    name: 'Manual finance operations',
    category: 'FINANCE',
  },
  ...
  {
    key: 'finance:withdrawal:create',
    name: 'Request organization withdrawals',
    category: 'FINANCE',
  },
  {
    key: 'finance:withdrawal:platform',
    name: 'Platform withdrawal operations',
    category: 'FINANCE',
  },
...
const PLATFORM_PERMISSIONS = [
  ...
  { key: 'event:approve', name: 'Approve events', category: 'EVENT' },
  {
    ...
    name: 'Create governance items',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:read',
    name: 'View governance items',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:update',
    name: 'Update governance items',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:delete',
    name: 'Delete governance items',
    category: 'GOVERNANCE',
  },
  {
  ...
  {
    key: 'governance:election',
    name: 'Elections overview',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:election:create',
    name: 'Create elections',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:election:manage',
    name: 'Manage elections',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:election:nominate',
    name: 'Nominate candidates',
    category: 'GOVERNANCE',
  },
  {
    key: 'governance:election:vote',
...
async function seedCatalog(pool) {
  ...
  console.log(
    `   Missing ${missing.length} key(s): ${missing.map((p) => p.key).join(', ')}`,
  );
  if (DRY_RUN) {
  ...
  for (const p of missing) {
    const res = await pool.query(
      ...
      [
        p.key,
        p.name,
        `${p.name} (auto-seeded by scripts/seed-permissions.js)`,
        p.category,
      ],
    );
...
async function syncAdminDefaults(pool) {
  ...
  if (admins.length === 0) {
    console.log(
      `⚠️  No matching ACTIVE admins found${ONLY_ADMIN_ID ? ` for id=${ONLY_ADMIN_ID}` : ''}.`,
    );
    return;
  }
  ...
  for (const admin of admins) {
    if (admin.adminType === 'PLATFORM_ADMIN') {
      console.log(
        `⏭️  Skipping PLATFORM_ADMIN ${admin.id} (guard bypasses checks for platform admins).`,
      );
      continue;
    }
    const defaults =
      DEFAULT_PERMISSIONS[admin.adminType] || DEFAULT_PERMISSIONS.DEFAULT;
    ...
    if (toAdd.length === 0) {
      console.log(
        `✅ ${admin.adminType} ${admin.id}: all ${defaults.length} defaults already present.`,
      );
      continue;
    }
    console.log(
      `➕ ${admin.adminType} ${admin.id}: adding ${toAdd.length} default(s): ${toAdd.join(', ')}`,
    );
    if (DRY_RUN) continue;
...
(async () => {
  ...
  try {
    ...
    if (DRY_RUN) {
      ...
      console.log(
        'ℹ️  Note: GET /rbac/permissions caches for 10 min; the UI list refreshes after that TTL or via cache invalidation/restart.',
      );
    }
  } catch (err) {
